...
#             pool_val=polling(torch.tensor(imgGray_val)).reshape(len(data.X_val),-1)
            ...
            num_label=model_info.data.num_labels
            stability,time_all,ex_in_time= stability_calc_pool(np.array(model_info.data.X_train),model_info.data.X_val,model_info.data.y_train,model_info.y_pred_val,num_labels,pool_type='Avg',pool_size=2) 
            stability_test,_,_= stability_calc_pool(np.array(model_info.data.X_train),model_info.data.X_test,model_info.data.y_train,model_info.y_pred_test,num_labels,pool_type='Avg',pool_size=2) 
